...
class _OwnerMessagesTabState extends State<OwnerMessagesTab> with AutomaticKeepAliveClientMixin { // UX-audit #3: preserve tab state
  @override
  ...
  Stream<List<Map<String, dynamic>>> _buildMessageStream() {
    ...
    final projectIds = _ownerProjects.map((p) => p['id']?.toString() ?? '').where((id) => id.isNotEmpty).toList(); // UX-audit CI-04
  ...
  Widget build(BuildContext context) {
    ...
    // PP-11: Constrain message width on wide screens for readability
              ...
              child: DropdownButtonFormField<String>(
                ...
                items: _ownerProjects.map((p) {
                  return DropdownMenuItem(
                    value: p['id']?.toString() ?? '', // UX-audit CI-04
          ...
          Expanded(
            child: _ownerProjects.isEmpty
                ? const EmptyStateWidget(
                    ...
                    action: Text('Voice notes from your team will appear here', style: TextStyle(color: Color(0xFF616161), fontSize: 13)),
                  )
